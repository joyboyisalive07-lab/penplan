...
В репозитории есть холст для тренировки: откройте в браузере
`tools/testcanvas.html`. Это обычный HTML-canvas с палитрой, двумя
инструментами и четырьмя размерами кисти, и он ничей — на нём можно прогнать
весь конвейер, не участвуя ни в чьей игре. Штатный профиль `test-canvas` сделан
по нему, и картинка в начале файла нарисована на нём.
