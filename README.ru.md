...
- **Калибруйтесь в приватном лобби, в своём раунде.** Экран рисования
  существует только пока идёт раунд, а калиброваться посреди чужой игры и
  невежливо, и некогда.
- **Размер кисти там ползунок.** Клик по ползунку переносит его в место клика,
  поэтому снимайте две-четыре точки вдоль него как если бы это были кнопки, от
  тонкой к толстой, и обязательно с `--measure`, чтобы планировщик узнал, что
  каждая из этих точек реально рисует.
